...
const MocktestForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    let {
      ...
      timing,
      totalMarks,
      maximumQuestionsAllowedToAttempt,
      difficulty,
    } = formData;
    let entranceExams = formData.entranceExams.split(",");
    let subjects = formData.subjects.split(",");

    const newMocktest = await fetch(
      `${API}/mocktest/createMocktest`,
      {
        method: "POST",
        headers: {
  ...
  return (
    <div className="formSubContainer">
      ...
      <form onSubmit={handleSubmit} className="formBox">
        ...
        <div>
          <label for="maximumQuestionsAllowedToAttempt">
            Maximum Questions Allowed To Attempt:
          </label>
          <input
            type="number"
            name="maximumQuestionsAllowedToAttempt"
            value={formData.maximumQuestionsAllowedToAttempt}
            onChange={handleChange}
            id="maximumQuestionsAllowedToAttempt"
          />
        </div>
