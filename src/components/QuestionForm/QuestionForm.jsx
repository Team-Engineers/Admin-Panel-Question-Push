...
const QuestionForm = () => {
    ...
    const handleChange = (e, index, fieldName, subFieldName, subIndex) => {
        ...
        if (subFieldName) {
            if (subFieldName === 'questionTextAndImages' || subFieldName === 'options' || subFieldName === 'explanation') {
                if (name === 'image') {
                    newFormData[fieldName][index][subFieldName][subIndex][name] = files[0];
                }
                else {
    ...
    const handleSubmit = async (e) => {
        ...
        await Promise.all(anArray.map(handleImageUpload))
            .then(async () => {
                ...
                let url = generalContext.mocktestId.length ? 'https://ourntamockpapers.onrender.com/api/mocktest/add-question' : 'https://ourntamockpapers.onrender.com/api/question/create-question';
    ...
    return (
        ...
        <div className='questionContainer'>
            {generalContext.mocktestId.length ? <h2>Add New Question in {generalContext.mocktestName}</h2> : <h2>Create New Question</h2>}

            <div style={{ width: '100%' }}>
                <FormControl >
                    <FormLabel id="demo-controlled-radio-buttons-group">Does Question have further subquestions ?</FormLabel>
                    <RadioGroup row
            ...
            <form onSubmit={handleSubmit} className='questionForm'>
                <div id="question-details">
                        ...
                        <FormControl fullWidth>
                            ...
                            >

                                {
                                    ...
                                    (
                                        <MenuItem value={slugify(diff, '_')} key={index}>{diff}</MenuItem>
                                    ))
                                }
                            </Select>
                        </FormControl>
                    ...
                    <div className='form-control'>
                        <FormControl>
                            <InputLabel id="demo-multiple-name-label">Entrance Exams</InputLabel>
                            <Select
                        ...
                        (<>
                            ...
                            <div>
                                ...
                                {formData.questionTextAndImages.map((item, index) => (
                                    <div key={index}>
                                        <textarea className='textInput' rows={8} name='text' placeholder='Enter Paragraph...' value={item.text} onChange={(e) => handleChange(e, index, 'questionTextAndImages')}></textarea>
                                        <input type="file" name="image" onChange={(e) => handleChange(e, index, 'questionTextAndImages')} />
                                    </div>
                                ))}
                                <button className='allButtons' type="button" onClick={() => handleAddField('questionTextAndImages', null, null)}>Add Paragraph Text and Image</button>
                            </div>

                        </>) : null}

                <div className='subQuestionContainer'>
                    <h3>{isSubquestion && "Sub"} Question{isSubquestion && "s"}:</h3>
                            ...
                            <div key={index} className='subQuestion'>
                                ...
                                <div id="question-details">
                                    <div className='form-control'>
                                        <FormControl fullWidth>
                                            ...
                                            <Select
                                                ...
                                                label="Difficulty"
                                                name='difficulty'
                                                onChange={(e) => handleChange(e, index, 'subQuestions', 'difficulty')}
                                            >
                                    ...
                                    <div>
                                        ...
                                        <input className='textInput' type="number" name="correctOptionIndex" value={subQuestion.correctOptionIndex} onChange={(e) => handleChange(e, index, 'subQuestions', 'correctOptionIndex')} />
                                    </div>

                                    <div>
                                        <label>Positive Marks:</label>
                                        <input className='textInput' type="number" name="positiveMarks" value={subQuestion.positiveMarks} onChange={(e) => handleChange(e, index, 'subQuestions', 'positiveMarks')} />
                                    </div>

                                    <div>
                                        <label>Negative Marks:</label>
                                        <input className='textInput' type="number" name="negativeMarks" value={subQuestion.negativeMarks} onChange={(e) => handleChange(e, index, 'subQuestions', 'negativeMarks')} />
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    {subQuestion.questionTextAndImages.map((item, idx) => (
                                        <div key={idx}>
                                            ...
                                            <textarea
                                                ...
                                                value={item.text}
                                                onChange={(e) => handleChange(e, index, 'subQuestions', 'questionTextAndImages', idx)}
                                            ></textarea>
                                            <input
                                                type="file"
                                                name="image"
                                    ...
                                    <button className='allButtons' type="button" onClick={() => handleAddQuestionTextAndImages(index)}>Add Question Text and Image</button>
                                ...
                                <div>
                                    ...
                                    {subQuestion.options.map((item, idx) => (
                                        <div key={idx}>
                                            <input className='textInput'
                                                ...
                                                value={item.text}
                                                onChange={(e) => handleChange(e, index, 'subQuestions', 'options', idx)}
                                            />
                                            <input
                                                type="file"
                                                name="image"
                                    ...
                                    <button className='allButtons' type="button" onClick={() => handleAddOption(index)}>Add Options</button>
                                    {/* End of input section for questionTextAndImages */}
                                </div>
                                ...
                                <div>
                                    ...
                                    {subQuestion.explanation.map((item, idx) => (
                                        <div key={idx}>
                                            ...
                                            <textarea
                                                ...
                                                value={item.text}
                                                onChange={(e) => handleChange(e, index, 'subQuestions', 'explanation', idx)}
                                            ></textarea>
                                            <input
                                                type="file"
                                                name="image"
                                    ...
                                    <button className='allButtons' type="button" onClick={() => handleAddExplanation(index)}>Add Explanations</button>
                                    {/* End of input section for questionTextAndImages */}
                                </div>
                    ...
                    {isSubquestion && <button className='allButtons' type="button" onClick={handleAddSubQuestion}>Add Sub Question</button>}
                </div>
